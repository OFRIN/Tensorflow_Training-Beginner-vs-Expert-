...
print('[i] Dataset')
print('[i] Train : {}'.format(len(train_thread.data_list)))
...
logits, predictions = fine_tuning(input_var, is_training)
...
train_iteration = len(train_thread.data_list) // BATCH_SIZE
...
for iter in range(1, max_iteration + 1):
    ...
    if iter % VALID_ITERATION == 0:
        ...
        for valid_iter in range(valid_iteration):
            batch_data = valid_data[valid_iter * BATCH_SIZE : (valid_iter + 1) * BATCH_SIZE]
...
print('[i] train time : {}sec'.format(train_time))
